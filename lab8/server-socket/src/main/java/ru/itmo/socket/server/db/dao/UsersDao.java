...
public class UsersDao {
    ...
    public UserDto insert(UserDto user) {
        String sql = "INSERT INTO s467392.users (login, password_hash) VALUES (?, ?) RETURNING id";
    ...
    public UserDto findByUsername(String login) {
        String sql = "SELECT id, password_hash FROM s467392.users WHERE login = ?";
        try (PreparedStatement st = DbUserContext.getConnection().prepareStatement(sql)) {
    ...
    public UserDto findById(int id) {
        String sql = "SELECT id, login FROM s467392.users WHERE id = ?";
        try (PreparedStatement st = DbUserContext.getConnection().prepareStatement(sql)) {
            st.setInt(1, id);
            ResultSet rs = st.executeQuery();
    ...
    public List<UserDto> findAll(Connection connection) {
        String sql = "SELECT id, login, password_hash FROM s467392.users";
